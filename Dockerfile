...
RUN apk update && apk add gcc cmake clang make linux-headers musl-dev ninja \
    curl-dev zlib-dev libjpeg-turbo-dev gmp-dev autoconf clang-dev \
    automake libtool binutils binutils-gold llvm8-static llvm8 llvm8-dev \
    g++ && mkdir /src && mkdir /build
...
RUN apk --no-cache add libjpeg-turbo libjpeg gmp libcurl libgcc libgc++
