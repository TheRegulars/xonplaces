...
int PRVM_SetTempString(prvm_prog_t *prog, const char *s)
{
    ...
    if (prog->tempstringsbuf.maxsize < prog->tempstringsbuf.cursize + size)
    {
        ...
        if (prog->tempstringsbuf.cursize + size >= (1 << 28 | 1 << 27))
            prog->error_cmd("PRVM_SetTempString: ran out of tempstring memory!  (refusing to grow tempstring buffer over 384MB, cursize %i, size %i)\n", prog->tempstringsbuf.cursize, size);
