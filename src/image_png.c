...
unsigned char *PNG_LoadImage_BGRA (const unsigned char *raw, int filesize, int *miplevel)
{
    ...
    if(png_sig_cmp(raw, 0, filesize))
        return NULL;
    png = png_create_read_struct(PNG_LIBPNG_VER_STRING, 0, PNG_error_fn, PNG_warning_fn);
    if(!png)
        return NULL;
...
qboolean PNG_SaveImage_preflipped (const char *filename, int width, int height, qboolean has_alpha, unsigned char *data)
{
    ...
    png = png_create_write_struct(PNG_LIBPNG_VER_STRING, 0, PNG_error_fn, PNG_warning_fn);
    if(!png)
        return false;
    pnginfo = png_create_info_struct(png);
    if(!pnginfo)
