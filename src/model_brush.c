...
static void Mod_Q1BSP_LoadTextures(sizebuf_t *sb)
{
    ...
    for (i = 0;i < nummiptex;i++)
    {
        ...
        if (cls.state != ca_dedicated)
        {
            ...
            {
                skinframe = R_SkinFrame_LoadExternal(va(vabuf, sizeof(vabuf), "textures/%s/%s", mapname, tx->name), TEXF_ALPHA | TEXF_MIPMAP | TEXF_ISWORLD | TEXF_PICMIP | TEXF_COMPRESS, false);
                if (!skinframe)
